import { Component, Input, Output, EventEmitter, ElementRef, OnInit, forwardRef, OnChanges } from '@angular/core';
...
export class SelectComponent implements OnInit, ControlValueAccessor, OnChanges {
  ...
  public ngOnChanges(data: any): any {
    if (data && data.active && data.active.currentValue) {
      this.currentItem = this.itemObjects.find(item => item.id == ((typeof data.active.currentValue[0] === 'object') ? data.active.currentValue[0].id : data.active.currentValue[0]));
    }
  }
  ...
  protected isActive(value: SelectItem): boolean {
    return this.currentItem ? this.currentItem.id === value.id : false;
  }
